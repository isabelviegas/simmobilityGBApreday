--[[
Model - Tour time of day for recreational tour
Type - MNL
...
local beta_arr_1_cos2pi = -2 -- -2.54
local beta_arr_1_cos4pi = -0.2  -- -0.7 -- -0.888
local beta_arr_1_cos6pi = 0
local beta_arr_1_sin2pi = 0.5  -- 0.141
local beta_arr_1_sin4pi = -0.8 -- -1.10
local beta_arr_1_sin6pi = 0

local beta_dep_1_cos2pi = -2    -- -2.54 --0.3 -- 0.761
local beta_dep_1_cos4pi = -0.2     -- 0.177
local beta_dep_1_cos6pi = 0
local beta_dep_1_sin2pi = 0.5   -- 0.141 -- -0.7-- -1.46
local beta_dep_1_sin4pi = -0.8  -- -1.10 -- -0.0257
...
local function sarr_1(t)
	t=t+3
	return beta_arr_1_sin2pi * math.sin(2*pi*t/24.) + beta_arr_1_cos2pi * math.cos(2*pi*(t-4)/24.) + beta_arr_1_sin4pi * math.sin(4*pi*(t-0.5-0.2-0.2-0.2)/24.) + beta_arr_1_cos4pi * math.cos(4*pi*(t+2.5)/24.) + beta_arr_1_sin6pi * math.sin(6*pi*t/24.) + beta_arr_1_cos6pi * math.cos(6*pi*t/24.)
end

local function sdep_1(t)
	t = t+3
	return beta_dep_1_sin2pi * math.sin(2*pi*t/24.) + beta_dep_1_cos2pi * math.cos(2*pi*(t-4)/24.) + beta_dep_1_sin4pi * math.sin(4*pi*(t-0.5-0.2-0.2-0.2)/24.) + beta_dep_1_cos4pi * math.cos(4*pi*(t+2.5)/24.) + beta_dep_1_sin6pi * math.sin(6*pi*t/24.) + beta_dep_1_cos6pi * math.cos(6*pi*t/24.)
end
