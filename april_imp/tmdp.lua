...
local beta_cons_walk = 9.9033
local beta_cons_bike = -0.99   +1
local beta_cons_drive1 = -2.5  +0.5
local beta_cons_share2 = -1.24 
local beta_cons_share3 = -2.14
local beta_cons_PT = 1.61               -0.5-0.5
local beta_cons_OD = 7                  -2 --on demand
local beta_cons_PR = 0
...
local beta_cbd_notdrive = 0.428     
local beta_cbd_drive1 = -1.72       
local beta_cbd_share2 = -0.835      
local beta_cbd_share3 = -1.36       
local beta_cbd_PT = 0.947           
...
local function computeUtilities(params,dbparams)
	...
	for i=1,2727 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_drive1 + tt_car[i]*beta_tt_drive1 + beta_cbd_drive1 * CBD[i] + 
	...
	for i=1,2727 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_share2 + tt_car[i]*beta_tt_share2 + beta_cbd_share2 * CBD[i] +
	...
	for i=1,2727 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_share3 + tt_car[i]*beta_tt_share3 + beta_cbd_share3 * CBD[i] + 
	...
	for i=1,2727 do
		V_counter = V_counter +1
		utility[V_counter] = beta_cons_PR + tt_PR_PT[i]*beta_tt_PT + tt_PR_car[i]*beta_tt_drive1 + beta_cbd_PR * CBD[i] + 
		beta_log * log(AREA[i]+exp(beta_employment)*EMP[i]+exp(beta_population)*POP[i]) + beta_cost * (cost_PR_PT[i] + cost_PR_car[i]) + beta_Boston * bool_to_number[ TOWN[i]==0 ]
	end
...
function choose_tmdp(params,dbparams)
	-- print("@tmdp.lua: in choose_tmdp")
	computeUtilities(params,dbparams) 
	-- print("@tmdp.lua: computed utilities")
	computeAvailabilities(params,dbparams)
	-- print("@tmdp.lua: computed availabilities")
...
function compute_logsum_tmdp(params,dbparams)
	-- print("@tmdp.lua: in compute_logsum_tmdp")
	computeUtilities(params,dbparams) 
	-- print("@tmdp.lua: computed utilities")
	computeAvailabilities(params,dbparams)
	-- print("@tmdp.lua: computed availabilities")
	-- print("@tmdp.lua: logsum is ", compute_mnl_logsum(utility, availability))
